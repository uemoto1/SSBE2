    ...
    do ie = 1, ne
        ...
        do ik = 1, gs%nk
            do ib = 1, gs%nb
                do jb = 1, gs%nb
                    ! write(*, *) "loop", ik, ib, jb
                    ! write(*, *) gs%occup(ib, ik), gs%occup(jb, ik)
                    if (gs%occup(ib, ik) < 1.0 .and. 1.0 < gs%occup(jb, ik)) then
                        ...
                        ! write(*,* ) "gs%volume", gs%volume
                        ! write(*,* ) "gs%d_matrix(ib, jb, 1, ik)", gs%d_matrix(ib, jb, 1, ik)
                        ! write(*,* ) "gs%omega(ib, jb, ik)", gs%omega(ib, jb, ik)
                        ! write(*,* ) "e", e
                        ! write(*,* ) "gamma", gamma
                        ! write(*,* ) "gs%nk", gs%nk
